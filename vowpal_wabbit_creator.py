...
class VWCreator(object):
    ...
    def create_df_vw(self, _df, is_train):
        df = _df.copy()
        df['datestr'] = map(lambda d:d.strftime('%Y%m%d'), df.date2)
